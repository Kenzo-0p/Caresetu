...
import { Activity, ArrowRight, CheckCircle2, FileText, HeartPulse, ShieldCheck, Siren, Upload } from "lucide-react";
    ...
    <section className="relative overflow-hidden rounded-[28px] border border-teal-100 bg-white p-6 shadow-[0_20px_55px_-30px_rgba(15,118,110,.3)] sm:p-8" data-testid="patient-welcome-card">
      ...
      <div className="relative flex flex-col justify-between gap-6 md:flex-row md:items-center">
        ...
        <div className="flex items-center gap-3 rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3">
          ...
          <div><p className="text-sm font-semibold text-slate-800">Profile ready</p><p className="text-xs text-slate-500">Private demo record · {profile.secure_id}</p></div>
        </div>
      </div>
    </section>
  );
...
function HealthProfileCard({ profile }: { profile: Profile }) {
  ...
  const save = useMutation({ mutationFn: () => apiPut<Profile>("/patient/profile", { name: form.name, age: form.age, mobile: form.mobile, location: form.location, emergency_contact: form.emergency_contact, existing_conditions: form.existing_conditions, allergies: form.allergies, medicines: form.medicines, previous_history: form.previous_history }), onSuccess: () => { queryClient.invalidateQueries({ queryKey: ["demo-state"] }); toast.success("Health profile saved"); } });
...
function ProfileForm({ form, setForm, onSave, saving }: { form: Profile; setForm: (profile: Profile) => void; onSave: () => void; saving: boolean }) {
  return <div className="grid gap-3 sm:grid-cols-2"><Input value={form.name} onChange={(event) => setForm({ ...form, name: event.target.value })} aria-label="Name" data-testid="profile-name-input" /><Input type="number" value={form.age} onChange={(event) => setForm({ ...form, age: Number(event.target.value) })} aria-label="Age" data-testid="profile-age-input" /><Input value={form.mobile} onChange={(event) => setForm({ ...form, mobile: event.target.value })} aria-label="Mobile" data-testid="profile-mobile-input" /><Input value={form.location} onChange={(event) => setForm({ ...form, location: event.target.value })} aria-label="Location" data-testid="profile-location-input" /><Input value={form.emergency_contact} onChange={(event) => setForm({ ...form, emergency_contact: event.target.value })} aria-label="Emergency contact" data-testid="profile-emergency-contact-input" /><Button onClick={onSave} disabled={saving} className="bg-teal-700 hover:bg-teal-800 sm:col-span-2" data-testid="profile-save-button">{saving ? "Saving…" : "Save health profile"}</Button></div>;
}
...
function DocumentsSection({ documents }: { documents: DocumentItem[] }) {
  ...
  const upload = useMutation({ mutationFn: () => apiUpload<DocumentItem>("/documents/upload", file as File), onSuccess: () => { setFile(null); queryClient.invalidateQueries({ queryKey: ["demo-state"] }); toast.success("Document preserved for review"); } });
  const verify = useMutation({ mutationFn: (id: string) => apiPost<DocumentItem>(`/documents/${id}/verify`), onSuccess: () => { queryClient.invalidateQueries({ queryKey: ["demo-state"] }); toast.success("Candidate fields marked as verified history"); } });
  return <section className="rounded-[24px] border border-slate-200 bg-white p-5" data-testid="documents-card"><SectionHeading eyebrow="MEDICAL DOCUMENTS" title="Keep reports together" description="Original files stay available. Any candidate fields remain unverified until reviewed." icon={FileText} /><div className="flex flex-col gap-3 rounded-2xl border border-dashed border-slate-300 bg-slate-50 p-4 sm:flex-row sm:items-center sm:justify-between"><div className="flex items-center gap-3"><div className="grid size-10 place-items-center rounded-xl bg-white text-teal-700 shadow-sm"><Upload size={18} /></div><div><p className="text-sm font-semibold text-slate-800">Upload a PDF or image report</p><p className="text-xs text-slate-500">For the demo, add “unreadable” to a filename to view the safe failure state.</p></div></div><div className="flex flex-col gap-2 sm:flex-row"><Input type="file" accept="application/pdf,image/jpeg,image/png,image/webp" onChange={(event) => setFile(event.target.files?.[0] || null)} className="max-w-[230px] bg-white text-xs" data-testid="document-file-input" /><Button onClick={() => file && upload.mutate()} disabled={!file || upload.isPending} className="bg-teal-700 hover:bg-teal-800" data-testid="document-upload-button">{upload.isPending ? "Reading…" : "Upload"}</Button></div></div><DocumentList documents={documents} onVerify={(id) => verify.mutate(id)} /></section>;
}

function DocumentList({ documents, onVerify }: { documents: DocumentItem[]; onVerify: (id: string) => void }) {
  ...
  return <div className="mt-4 space-y-2">{documents.map((document) => <div key={document.id} className="flex flex-col gap-3 rounded-xl border border-slate-200 p-3 sm:flex-row sm:items-center sm:justify-between" data-testid={`document-${document.id}-row`}><div className="flex min-w-0 items-center gap-3"><FileText className="shrink-0 text-slate-400" size={18} /><div className="min-w-0"><p className="truncate text-sm font-medium text-slate-800">{document.filename}</p><p className="text-xs text-slate-500">Original preserved · {(document.size_bytes / 1024).toFixed(1)} KB</p>{document.extracted_fields.length > 0 && <p className="mt-1 text-xs text-slate-600">Candidate fields: {document.extracted_fields.join(" · ")}</p>}</div></div><div className="flex min-w-0 flex-wrap items-center gap-2"><Badge className={statusTone(document.review_status)}>{documentLabel(document)}</Badge>{document.review_status === "needs_verification" && <Button size="sm" variant="outline" onClick={() => onVerify(document.id)} data-testid={`document-${document.id}-verify-button`}>Review & verify</Button>}</div></div>)}</div>;
}

export default function PatientWorkspace({ state }: { state: DemoState }) {
  const queryClient = useQueryClient();
  const [assessmentOpen, setAssessmentOpen] = useState(false);
  const [sos, setSos] = useState<SOSResponse | null>(null);
  const [selectedFacility, setSelectedFacility] = useState("");
  const startSos = useMutation({ mutationFn: () => apiPost<SOSResponse>("/sos", { note: "Patient initiated the demo SOS flow." }), onSuccess: (response) => { setSos(response); queryClient.invalidateQueries({ queryKey: ["demo-state"] }); } });
  const notifyHospital = useMutation({ mutationFn: () => apiPost<SOSResponse>("/sos/select-facility", { sos_id: sos?.event.id, facility_id: selectedFacility }), onSuccess: (response) => { setSos(response); queryClient.invalidateQueries({ queryKey: ["demo-state"] }); toast.success("Hospital notified"); } });
  const beginSos = () => { setSos(null); setSelectedFacility(""); startSos.mutate(); };

  return <div className="space-y-6" data-testid="patient-workspace"><PatientHero profile={state.profile} /><PrimaryActions onAssessment={() => setAssessmentOpen(true)} onSos={beginSos} sosPending={startSos.isPending} />{sos && <EmergencyPanel response={sos} selectedFacility={selectedFacility} onSelect={setSelectedFacility} onNotify={() => notifyHospital.mutate()} notifying={notifyHospital.isPending} />}{assessmentOpen && <SymptomAssessment onClose={() => setAssessmentOpen(false)} />}<section className="grid gap-4 lg:grid-cols-[1.2fr_.8fr]"><HealthProfileCard profile={state.profile} /><ReferralSummary referral={state.referrals[0]} /></section><DocumentsSection documents={state.documents} /></div>;
}